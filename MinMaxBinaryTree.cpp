...
pair<int, int> getMinAndMax(BinaryTreeNode<int> *root) {
	...
	pair<int,int> h = getMinAndMax(root->left);
	pair<int,int> g = getMinAndMax(root->right);
	pair<int,int>gh;
	gh.first = min(root->data,min(h.first,g.first));
	gh.second = max(root->data,max(h.second,g.second));
	return gh;
}